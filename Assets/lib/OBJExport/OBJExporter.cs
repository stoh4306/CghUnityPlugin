...
{
    public static bool onlySelectedObjects = false;
    public static bool applyPosition = false;
    public static bool applyRotation = false;
    public static bool applyScale = false;
    public static bool generateMaterials = true;
    public static bool exportTextures = true;
    public static bool splitObjects = true;
    public static bool autoMarkTexReadable = true;
    public static bool objNameAddIdNum = false;
    ...
    string Export(string exportPath)
    {
        ...
        for (int i = 0; i < sceneMeshes.Length; i++)
        {
            ...
            if(mr != null && generateMaterials)
            {
                ...
                for(int j=0; j < mats.Length; j++)
                {
                    ...
                    if (!materialCache.ContainsKey(m.name))
                    {
                        ...
                        sbMaterials.Append(MaterialToString(m,ref textureFileName));
            ...
            for (int j=0; j < msh.subMeshCount; j++)
            {
                ...
                {
                    int idx0 = tris[t] + 1 + lastIndex;
                    int idx1 = tris[t + 1] + 1 + lastIndex;
                    int idx2 = tris[t + 2] + 1 + lastIndex;
                    if(faceOrder < 0)
                    {
    ...
    string MaterialToString(Material m, ref string textureFileName)
    {
        StringBuilder sb = new StringBuilder();
        string matName = m.name;
        string[] split = { "(", ")" };
        matName = matName.Split(split, System.StringSplitOptions.RemoveEmptyEntries)[0];
        ...
        {
            Color sc = m.color;
            if (m.HasProperty("_DiffuseFactor"))
            {
                sc*=m.GetFloat("_DiffuseFactor");

            }
            sb.AppendLine("Kd " + sc.r.ToString() + " " + sc.g.ToString() + " " + sc.b.ToString());
            if (m.color.a < 1.0f)
            {
        ...
        if (m.HasProperty("_SpecColor"))
        {
            ...
            if (m.HasProperty("_SpecularFactor"))
            {
                sc *= m.GetFloat("_SpecularFactor");
            }
            sb.AppendLine("Ks " + sc.r.ToString() + " " + sc.g.ToString() + " " + sc.b.ToString());
        }
        if (m.HasProperty("_AmbientFactor"))
        {
            Color sc = Color.white;
            sc *= m.GetFloat("_AmbientFactor");
            sb.AppendLine("Ka " + sc.r.ToString() + " " + sc.g.ToString() + " " + sc.b.ToString());
        }
        if (m.HasProperty("_Shininess")) {
            sb.AppendLine("sharpness " + m.GetFloat("_Shininess"));
        }


        if (exportTextures) {
            //diffuse
            string exResult = Path.GetFileName( TryExportTexture("_MainTex", m));
            if (exResult != "false")
            {
                sb.AppendLine("map_Kd " + exResult);
                textureFileName = exResult;
            }
            //spec map
    ...
    public string Export(GameObject targetObj,string exportPath)
    {
        //init stuff
        Dictionary<string, bool> materialCache = new Dictionary<string, bool>();
        var exportFileInfo = new System.IO.FileInfo(exportPath);
