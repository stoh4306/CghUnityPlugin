...
Shader "Hol/PhongShader" {
	Properties{
		_Color("Color", Color) = (1, 1, 1, 1) //The color of our object
		_MainTex("MainTexture", 2D) = "white" {} //Optional texture

		_Shininess("Shininess", Float) = 20
		_AmbientFactor("Ambient Factor",Float) = 0.2
		_DiffuseFactor("Diffuse Factor",Float) = 1
		_SpecularFactor("Specular Factor",Float) = 1 //Shininess
		...
		SubShader{
			...
			Pass {
				...
				CGPROGRAM
					...
					sampler2D _MainTex; //Used for texture
					...
					fixed4 frag(v2f i) : COLOR
					{
						...
						float3 ambientLighting = _AmbientFactor * _Color.rgb; //UNITY_LIGHTMODEL_AMBIENT.rgb * _Color.rgb; //Ambient component
						float3 diffuseReflection = _DiffuseFactor * attenuation * _LightColor0.rgb * _Color.rgb * max(0.0, dot(normalDirection, lightDirection)); //Diffuse component
						...
						specularReflection *= _SpecularFactor;

						float3 color = (ambientLighting + diffuseReflection) * tex2D(_MainTex, i.uv) + specularReflection; //Texture is not applient on specularReflection
			...
			Pass {
			  ...
			  fixed4 frag(v2f i) : COLOR
			  {
				  ...
				  float3 color = (diffuseReflection)* tex2D(_MainTex, i.uv) + specularReflection; //No ambient component this time
				  return float4(color, 1.0);
			  }
		  ENDCG
			}
